...
def compute_forces(positions, box_size):
    ...
    for i in range(N):
        for j in range(i + 1, N):
            ...
            V_total_pair = V_harmonic # + V_morse + V_LJ
            F_total_pair = F_harmonic # + F_morse + F_LJ # Sum the magnitudes of forces
...
def verlet_step(positions, velocities, accelerations, dt, box_size, damping_coefficient):
    ...
    velocities_new -= damping_coefficient * velocities_new

    return positions_new, velocities_new, accelerations_new

def simulate(positions, velocities, damping_coefficient):
    positions_history = []
    kinetic_energies = []
    potential_energies = []
    total_energies = []
    ...
    for step in range(num_steps):
        positions, velocities, accelerations = verlet_step(
            positions, velocities, accelerations, dt, box_size, damping_coefficient)
...
positions, KE, PE, TE = simulate(positions, velocities, damping_coefficient)
